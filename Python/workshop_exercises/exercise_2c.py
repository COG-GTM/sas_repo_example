"""
MEPS Workshop Exercise 2c: Narcotic Analgesics Purchases and Expenses, 2018
...
Input file: C:/MEPS/h206a.sas7bdat or h206a.dta (2018 Prescribed Medicines file)

This is the Python equivalent of the SAS program Exercise2c.sas
"""
...
def main():
    # Read in data from 2018 Prescribed Medicines file (HC-206A)
    # For 2018+, .dta or .sas7bdat formats are recommended
    h206a = load_meps_data("C:/MEPS/h206a.dta")
    ...
    narcotics = h206a[h206a['TC1S1_1'].isin(NARCOTIC_TC)].copy()
    
    print("=" * 60)
    print("NARCOTIC ANALGESICS PURCHASES AND EXPENSES, 2018")
    print("=" * 60)
    ...
    person_summary = narcotics.groupby(
        ['DUPERSID', 'VARSTR', 'VARPSU', 'PERWT18F']
    ).agg({
        'RXXP18X': ['sum', 'count']
    }).reset_index()
    
    person_summary.columns = ['DUPERSID', 'VARSTR', 'VARPSU', 'PERWT18F',
